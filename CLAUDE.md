...
- ✅ **Phase 8.2 complete (2026-05-08):** Driver Style Inspector — slide-in panel (`src/components/DriverStylePanel.tsx`) accessible from both Sandbox and Optimizer pages via "VIEW STYLE →" link below the driver dropdown. Panel content: comparison driver dropdown (defaults to next-slower by pace rank), Recharts RadarChart (6 axes: Pace, Tire Saving, Wet Skill, Smoothness, Aggression, Consistency — all normalized min/max across season drivers; ego=red/30% fill, comparison=grey/12% fill overlaid), auto-generated insight line (3 cases: faster by >5 ranks / better tire manager / similar profiles), two-column driver detail (cluster badge, 5 metric bars with normalized fill, sector profile S1–S3 in green/white, pace trends per compound). Closes on × button, Escape, or mousedown outside (uses document mousedown listener so form elements behind panel still receive click events). Wet Skill invert: more-negative delta (faster in wet) maps to higher normalized bar.
- ⏭️ Phase 8.3 next — Historical Race page + overall polish. See `ROADMAP.md`.
