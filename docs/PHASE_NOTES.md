...
## Phase 7.2 — Sandbox Mode UI (2026-05-08)
**Built:** Full Sandbox page at `/sandbox`. 40/60 split. Season-aware circuit + driver dropdowns (Zustand cache keyed by year). 3-step form with up to 4 pit stops. SIMULATE + GET AI PICK. Results panel with stat cards, stint bar, lap time chart (Recharts ComposedChart with ReferenceArea per compound + ReferenceLine per pit), stint summary table. Backend: season endpoints, `_validate_and_fix_strategy()`, `_ppo_note()` for agent quality, position gain sanity cap (`max_gain = (20 - start) * 0.4`).
**Worked well:** Season-aware cache in Zustand (fetch once per year, reuse) worked cleanly. Splitting confidence (data coverage) from ppo_note (agent quality) was the right call — lets the UI communicate two distinct signals.
**Pain points:** Multiple rounds of bug fixes from live testing — OOD never-pit (PPO trained on Bahrain, recommends 0 stops elsewhere), negative deg/lap (track evolution means model can decrease over stint, clamped to 0), 19-driver grids crashing GridRaceEnv (fixed in Phase 8.1).

## Phase 8.1 — Optimizer Mode UI (2026-05-08)
**Built:** Full Optimizer page at `/optimizer`. 35/65 split. Season-aware form identical pattern to Sandbox. Two actions: OPTIMIZE STRATEGY (PPO Grid agent) + SIMULATE MY STRATEGY (modal pit configurator). Loading spinner with 4 cycling messages at 1.5s intervals. 4-section results layout: strategy header (mode badge + confidence + strategy_rationale), stat cards + stint bar + lap time chart, Grid Position Tracker (hero viz), Rival Predictions + Undercut Windows.
**Worked well:** Grid Position Tracker with 20 Recharts Line components renders clearly — ego red/thick on top (last rendered = highest z), rivals grey/thin underneath, inverted Y-axis (P1 at top). Custom tooltip showing top-6 by position keeps it readable. Strategy rationale from backend is specific and reads like real F1 language (names rivals, cites tire ages, references tire saving coefficient). Undercut window detection renders with correct gap data.
**Pain points:** Historical grids can have <20 drivers (Australian GP 2024 has 19 — SAR likely filtered from raw data). GridRaceEnv hard-asserts exactly 20 → 500 crash. Frontend now pads from season roster. Also: two uvicorn processes from different sessions caused CORS failure (port 5174 not in old instance's allow list) — always kill old processes before starting new backend.
**Rival position interpolation:** rivals have only start/end/pit_history (no per-lap positions from GridRaceEnv) — interpolate linearly with +4 position dip at each pit lap. Produces realistic-looking trajectories for the chart without per-lap simulation data.
**Open:** Phase 8.2 — Driver Style Inspector.

## Phase 8.2 — Driver Style Inspector (2026-05-09)
**Built:** Slide-in `DriverStylePanel` component (`frontend/src/components/DriverStylePanel.tsx`, ~280 lines) accessible from both Sandbox and Optimizer pages via a "VIEW STYLE →" link below the driver dropdown.
- **RadarChart (Recharts):** 6 axes — Pace, Tire Saving, Wet Skill, Smoothness, Aggression, Consistency. All metrics min/max normalised across the active season's driver set. Pace and Wet Skill axes are inverted (lower rank / more-negative delta = better). Consistency is a 0.5 placeholder (no stdev data available). Ego fill red/30%, comparison fill grey/12% overlaid.
- **Comparison dropdown:** defaults to the next-slower driver by `overall_pace_rank`; user can switch to any other driver in the season.
- **Auto-generated insight line:** 3-case logic — pace difference >5 ranks → "X is significantly faster"; tire saving difference >0.003 → "X is the better tire manager"; else "similar profiles."
- **Two-column driver detail:** cluster badge (5-label map), 5 MetricBar rows (10-segment bar, ego=red fill, comparison=grey fill), sector profile S1/S2/S3 (green for negative = faster, white otherwise), pace trends per compound (soft/medium/hard).
