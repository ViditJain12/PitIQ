...
# Verify that ingest_season returns a pandas DataFrame.
def test_returns_dataframe(single_race_df: pd.DataFrame):
    assert isinstance(single_race_df, pd.DataFrame)


# Verify that all expected lap columns are present in the ingested DataFrame.
...
# Verify that the row count is plausible for a 57-lap 20-driver race.
...
# Verify that LapTime values are floats in the expected 50–200 second range.
...
# Verify that telemetry summary columns are stored as float dtype.
...
# Verify that ingest_season writes laps_2024.parquet to the processed data directory.
...
# Verify that the written parquet file can be read back and contains rows.
